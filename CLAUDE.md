...
45. **Carillon** (slug `Carillon`, called Peal until 2026-09-25, hidden, first pass 2026-09-22) — A set of bells joined by rods (26HP, screenless, drawn on the faceplate from a foreshortened angle like Wheel and Flock). **A bell IS a note**: bell n is semitone n − 12 from ROOT, 25 unique semitones, whatever the layout; V/OCT folds by octaves into the two octaves and strikes THAT bell, felted or not (`bellForSemis()`; the old Tonnetz fold left notes with no bell and the old nearest-bell lookup skipped felted ones, so two notes of a melody struck one bell — the harness now strikes all 49 notes from −24 to +24 at two roots). Rods are stored by bell (`rodM[25×25]`, JSON `"rods"` pairs), so they survive a layout change. **Layouts are physical** (`relayout()`: Grid as a snake, Keyboard following the root, Ring, Spiral, seeded Scatter), a path of 25 positions with a **tuning order** along it (by pitch, by fifths via the fixed path `PL_FIFTHS`, shuffled; the keyboard has none). **A crossing's time is the rod's length**: SPEED (4 s..10 ms) per neighbour spacing (`spacing` = median nearest-neighbour distance, `rodLen()`), and with CLOCK patched `rodPulses()` = max(1, round(length)) pulses, landing on the NEXT pulse for a neighbour (`pulseAt = pulseCount + pulses − 1`). **REACH is transmission** (`carillonTrans` = 0.95·k², split between the rods, conserved, fixed floor `PL_FLOOR` 2e-4) and **energy ripples outward** (never back down the rod it arrived on): 1/4/7/16/25 bells along the chain at REACH 0/.25/.5/.75/1. Rod presets on the path and pitches: chain, neighbours, star, octaves, fifths, clear. **17-partial bell** (`PL_RATIO/PL_DB/PL_T60`: hum/prime/tierce/quint/nominal + upper partials, doublets on hum, prime, tierce, nominal; levels as heard so no radiation tilt; T60 30 s hum to 0.8 s top; pitch-defining partials within 9 cents of true and each doublet's weak member either side, so the strike note is in tune; **SIZE moves pitch in whole octaves only** (`carillonSizeOct`, it used to detune continuously) and ring time continuously; **DAMP is a ring-time multiplier** (`carillonRing`: 4x as cast at 0, as cast at 27%, default 36%); mics use `1/sqrt(1+d)` and the outs are `sfs::softClip`ped, since the full 1/d law plus a clamp distorted front-row chords), SHAPE morphs to a free bar (`PL_BAR_*`, which alone takes the √f radiation tilt), BRIGHT tilts and shortens the raised-cosine mallet contact; rung-out bells (`quiet`) skip their modes. ROOT/SCALE FELT out-of-key bells (0.2 amplitude, 8× damping, still conduct; `unmask` lifts them). Two draggable mics (per-bell delay lines, 1/distance gain); POLY = the 16 loudest bells. Display: circles (larger for lower notes, back to front, orange fill when struck, a ring while sounding, felted hollow), hit test = the bell the pointer is most nearly central in (`bellAt`), hover label drops below a bell that would clip the top; drag bell→bell lays/lifts a rod, click strikes, ant trails show impulses in flight. `tools/carillon-harness.cpp` checks V/OCT mapping, reach vs REACH and velocity, runaway, clocked and multi-pulse crossings, free-running length ratio, felts, resonance, mics, SHAPE/BRIGHT, hover + projection round-trip in every layout, and cost; `render DIR` writes listening WAVs. Design in `docs/carillon-design.md`.
...
- `src/field.cpp` — Field
- `src/carillon.cpp` — Carillon (was Peal; hidden, screenless; `tools/carillon-harness.cpp` = V/OCT mapping, reach, runaway, clocked and multi-pulse crossings, felts, resonance, mics, hover in every layout and cost against libRack; `render DIR` writes listening WAVs)
...
- `src/fastmath.hpp` — Memo + polynomial sin/exp/exp2/log2/tanh/pow for the audio thread (see Performance)
- `tools/perf/` — CPU benchmark for every module, render mode, `compare.py` / `compare-stats.py` (see Performance)
...
### Performance
**Measure before and after, and prove the sound did not move.** `tools/perf/` builds every module with the plugin's own flags against the installed libRack and times `process()` idle, under a generic stimulus (4 Hz gates, a V/OCT melody, noise on audio ins) and with eight channels on every gate and V/OCT; figures are % of one core at 48 kHz, which is what Rack's meter shows:

```bash
tools/perf/build.sh && tools/perf/build/bench            # every module, ~5 s
BENCH_CASE=poly8 BENCH_SECONDS=300 tools/perf/build/bench Kit &   # then: sample <pid> 5
```

An optimisation is verified by **render comparison against the last commit**: build the reference tree (`git archive HEAD src res | tar -x -C <dir>`, copy `tools/perf` in, `RACK_SDK=… <dir>/tools/perf/build.sh`), render both with `BENCH_RENDER=out.raw BENCH_CASE=stim bench <Slug>` (Rack's random generator is seeded, so both builds draw the same numbers), and run `tools/perf/compare.py ref.raw new.raw`. Where a change legitimately alters the random draws (a voice that sleeps skips its draws), a sample comparison is meaningless: render several seeds of each (`BENCH_SEED`) and run `tools/perf/compare-stats.py 'ref-*.raw' 'new-*.raw'`, which tests the MEAN level and octave bands against the spread between performances. Slide's sleeping strings looked like a 5.6 dB band change and were 0.22 dB against a 1 dB noise floor.

What the 2026-09-25 pass found and fixed (library busy total 20.0% → 14.7%): **libm in the per-sample loop** was 50-85% of eight modules. `src/fastmath.hpp` (the MetaModule port's helpers, fast on desktop too) gives `sfs::Memo`/`Memo2` for anything that only moves with a knob or the sample rate (exact), and `SFS_SIN2PI`/`SFS_COS2PI` (-108 dB), `SFS_EXP2` (0.003 cents), `SFS_EXP`/`SFS_TANH`/`SFS_POWABS`/`SFS_LOG2` (1e-4, for decays, saturation and shapes, not pitch). `sfs::SVF::set()` memoises its `tan`; `setFast()` is for a filter that really moves every sample. **Silent voices sleep** (Slide, Loom: below -80 dB, bridge-woken strings on probation). Constant per-voice factors are hoisted (Chime's partial decays, Flock's per-bird decays, Intone's per-grain `e^-alpha`), and oscillators that compute every wave shape compute only the two being blended (Wheel). Second pass the same day: Sigma resolves each partial's level, pitch and pan every 16 samples (the MetaModule port's scheme) EXCEPT while the envelope is routed in the matrix and in attack/decay/release, because Acid's 2 ms envelope sweeping a resonant cutoff three octaves jumped the peak half an octave per resolve (3 dB in an octave band; ramping made it worse, since the lag hurt the pluck); every preset now matches to -54 dB or better except Psychedelic Gong, which is phase-only (level identical, bands within 0.34 dB). GSX (double sin/cos per grain, pan gains per grain), Drift, Operator (output tanh), Crystal (exciter decays, loop tanh with slope 1 at zero), Spool. Gravity's chaotic core was deliberately left in double precision -- any change to it moves the trajectory -- so only output-side trig and knob maths changed. Library totals: idle 14.1% → 9.5%, busy 20.0% → 13.8%, poly8 30.9% → 21.8%. Peal (now Carillon) was left for later on purpose. Two measurement traps: Crystal is not deterministic run to run (its tracer is a worker thread), so compare it against a second run of the reference, not against zero; and PolyKit In's ~1% at poly8 is the benchmark writing 896 input channels a sample, not the module.
